...
Java_com_bgmi_utils_Downtwo_Version(JNIEnv *env, jclass clazz) {
    ...
    const char *versionUrl = (oxorany("https://github.com/godbgmi689-web/RABLOADER/releases/download/RAB/version.txt"));
...
Java_com_bgmi_utils_Downtwo_Link(JNIEnv *env, jclass clazz) {
    ...
    const char *downloadUrl = (oxorany("https://github.com/godbgmi689-web/RABLOADER/releases/download/RAB/V4.zip")); //Last Mai Apka Zip Name
...
Java_com_bgmi_BoxApplication_getSdkKey(JNIEnv *env, jclass clazz) {
    return env->NewStringUTF(oxorany("UOFU2AEXO"));//sdk key 
}
...
Java_com_bgmi_LogAct_Check(JNIEnv *env, jclass clazz, jobject mContext, jstring mUserKey) {
    ...
    if (curl) {
        char lol[1000];
      sprintf(lol,oxorany("https://tapatap.ghostpanel.in/connect")); 
